...
@task_router.get("/assigned/{id}", response_model=List[TaskResponse])
async def get_assigned_tasks(user_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    repo = Repository(db)
    service = Service(repo)
